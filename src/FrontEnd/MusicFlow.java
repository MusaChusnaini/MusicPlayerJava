...
public class MusicFlow extends JFrame {
    ...
    private JPanel createSidebar() {
        ...
        JLabel p1 = new JLabel("Premium Features"); p1.setForeground(Color.WHITE); p1.setFont(new Font("SansSerif", Font.BOLD, 12));
        JLabel p2 = new JLabel("Unlimited songs"); p2.setForeground(new Color(255,255,255,180)); p2.setFont(new Font("SansSerif", Font.PLAIN, 10));
        ...
        JPanel badge = new JPanel(new BorderLayout()) {
            protected void paintComponent(Graphics g) {
                ...
                g2.setPaint(gp); g2.fillRoundRect(0,0,getWidth(),getHeight(), 15, 15);
            }
        };
        badge.setOpaque(false); badge.setBorder(new EmptyBorder(15,15,15,15)); badge.setMaximumSize(new Dimension(200, 70));
        badge.add(pPanel, BorderLayout.CENTER); badge.setAlignmentX(LEFT_ALIGNMENT);
        sidebar.add(badge);
        return sidebar;
    }
    ...
    class PlayerPanel extends JPanel {
        ...
        private JPanel createFullPlaybackBar() {
            ...
            JLabel lblStart = new JLabel("0:45"); lblStart.setForeground(COL_TEXT_SEC);
            JLabel lblEnd = new JLabel("4:20"); lblEnd.setForeground(COL_TEXT_SEC);
            ...
            progressBar.setUI(new BasicProgressBarUI() { protected Color getSelectionBackground() { return COL_ACCENT; } protected Color getSelectionForeground() { return COL_ACCENT; } });
    ...
    class LibraryPanel extends JPanel {
        public LibraryPanel(MusicFlow frame) {
            ...
            JLabel title = new JLabel("Your Library"); title.setFont(new Font("SansSerif", Font.BOLD, 32)); title.setForeground(COL_TEXT);
            add(title, BorderLayout.NORTH);
            String[] cols = {"#", "TITLE", "ARTIST", "ALBUM", "DURATION", ""};
            Object[][] data = { {"1", "Midnight Dreams", "Luna Echo", "Nocturnal", "4:05", "⋮"}, {"2", "Electric Sunrise", "Nova Beats", "Dawn", "3:18", "⋮"}, {"3", "Vintage Vibes", "Retro Soul", "Classic", "3:43", "⋮"}, {"4", "Ocean Waves", "Coastal Dreams", "Blue Horizon", "4:27", "⋮"}, {"5", "City Lights", "Urban Symphony", "Metropolitan", "3:21", "⋮"} };
            ...
            JScrollPane scroll = new JScrollPane(table); scroll.getViewport().setBackground(COL_BG_MAIN); scroll.setBorder(null);
            add(scroll, BorderLayout.CENTER);
        }
    }

    class PlaylistManagerPanel extends JPanel {
        private CardLayout plLayout = new CardLayout(); private JPanel plContent = new JPanel(plLayout); private MusicFlow mainFrame;
        public PlaylistManagerPanel(MusicFlow frame) { this.mainFrame = frame; setLayout(new BorderLayout()); add(plContent, BorderLayout.CENTER); plContent.add(new PlaylistGrid(), "GRID"); plLayout.show(plContent, "GRID"); }
        class PlaylistGrid extends JPanel {
            public PlaylistGrid() {
                setLayout(new BorderLayout()); setBackground(COL_BG_MAIN); setBorder(new EmptyBorder(40, 40, 40, 40));
                JPanel header = new JPanel(new BorderLayout()); header.setBackground(COL_BG_MAIN); JLabel title = new JLabel("Your Playlists"); title.setFont(new Font("SansSerif", Font.BOLD, 32)); title.setForeground(COL_TEXT);
                PurpleButton btnCreate = new PurpleButton("+ Create Playlist"); btnCreate.addActionListener(e -> showCreateDialog()); header.add(title, BorderLayout.WEST); header.add(btnCreate, BorderLayout.EAST); add(header, BorderLayout.NORTH);
                JPanel grid = new JPanel(new FlowLayout(FlowLayout.LEFT, 30, 30)); grid.setBackground(COL_BG_MAIN);
                grid.add(createCard("Chill Vibes", "Relaxing beats", new Color(100, 50, 100), () -> openDetail("Chill Vibes", "Relaxing beats")));
                grid.add(createCard("Workout Energy", "Power up!", new Color(50, 50, 100), () -> openDetail("Workout Energy", "Power up!"))); add(grid, BorderLayout.CENTER);
            }
        ...
        class PlaylistDetail extends JPanel {
            public PlaylistDetail(String titleStr, String descStr) {
                setLayout(new BorderLayout()); setBackground(COL_BG_MAIN); setBorder(new EmptyBorder(40, 40, 0, 40));
                JPanel header = new JPanel(new BorderLayout()); header.setBackground(COL_BG_MAIN); JButton btnBack = new JButton("← Back to Playlists"); btnBack.setForeground(COL_ACCENT); btnBack.setContentAreaFilled(false); btnBack.setBorderPainted(false); btnBack.setFont(new Font("SansSerif", Font.BOLD, 14)); btnBack.setCursor(new Cursor(Cursor.HAND_CURSOR)); btnBack.addActionListener(e -> plLayout.show(plContent, "GRID"));
                JPanel info = new JPanel(); info.setLayout(new BoxLayout(info, BoxLayout.Y_AXIS)); info.setBackground(COL_BG_MAIN); JLabel title = new JLabel(titleStr); title.setFont(new Font("SansSerif", Font.BOLD, 40)); title.setForeground(Color.WHITE); info.add(title); header.add(btnBack, BorderLayout.NORTH); header.add(info, BorderLayout.CENTER); add(header, BorderLayout.NORTH);
                String[] cols = {"#", "TITLE", "ARTIST", "ALBUM", "DURATION", ""}; Object[][] data = {{"1", "Song A", "Artist A", "Album A", "3:00", "⋮"}, {"2", "Song B", "Artist B", "Album B", "4:20", "⋮"}}; DefaultTableModel model = new DefaultTableModel(data, cols) { public boolean isCellEditable(int r, int c) { return false; } }; JTable table = new JTable(model);
                setupTableLogic(table, model, this, (t, a) -> mainFrame.playSong(t, a)); JScrollPane scroll = new JScrollPane(table); scroll.getViewport().setBackground(COL_BG_MAIN); scroll.setBorder(BorderFactory.createEmptyBorder(20,0,20,0)); add(scroll, BorderLayout.CENTER);
            }
        }
        private void showCreateDialog() {
            JDialog dialog = new JDialog((Frame)SwingUtilities.getWindowAncestor(this), "Create Playlist", true); dialog.setUndecorated(true); dialog.setSize(400, 320); dialog.setLocationRelativeTo(this);
            ...
            JLabel l = new JLabel("Create New Playlist"); l.setForeground(Color.WHITE); l.setFont(new Font("SansSerif", Font.BOLD, 20)); l.setAlignmentX(LEFT_ALIGNMENT);
            ...
            PurpleButton btn = new PurpleButton("Create"); btn.setAlignmentX(LEFT_ALIGNMENT); btn.addActionListener(e->dialog.dispose()); JButton cancel = new JButton("Cancel"); cancel.setContentAreaFilled(false); cancel.setBorderPainted(false); cancel.setForeground(Color.GRAY); cancel.addActionListener(e->dialog.dispose());
    ...
    class AddMusicPanel extends JPanel {
        public AddMusicPanel() {
            setLayout(new BorderLayout()); setBackground(COL_BG_MAIN); JPanel content = new JPanel(); content.setLayout(new BoxLayout(content, BoxLayout.Y_AXIS)); content.setBackground(COL_BG_MAIN); content.setBorder(new EmptyBorder(40, 60, 40, 60));
            ...
            JPanel form = new JPanel(new GridLayout(2, 2, 20, 20)); form.setBackground(COL_BG_MAIN); form.setMaximumSize(new Dimension(800, 140)); form.setAlignmentX(LEFT_ALIGNMENT); form.add(createInput("Song Title")); form.add(createInput("Artist")); form.add(createInput("Album")); form.add(createInput("Duration")); content.add(form); content.add(Box.createVerticalStrut(30));
            JPanel upload = new JPanel(); upload.setBackground(COL_BG_MAIN); upload.setBorder(BorderFactory.createDashedBorder(Color.GRAY, 2, 5, 5, true)); upload.setMaximumSize(new Dimension(800, 100)); upload.add(new JLabel("Drag & Drop Audio File Here"){{setForeground(Color.GRAY); setFont(new Font("SansSerif", Font.BOLD, 14));}}); upload.setAlignmentX(LEFT_ALIGNMENT); content.add(upload); content.add(Box.createVerticalStrut(30));
            PurpleButton btnAdd = new PurpleButton("Add to Library"); btnAdd.setAlignmentX(LEFT_ALIGNMENT); content.add(btnAdd); add(content, BorderLayout.CENTER);
        }
        private JPanel createInput(String label) { JPanel p = new JPanel(new BorderLayout()); p.setBackground(COL_BG_MAIN); JLabel l = new JLabel(label); l.setForeground(COL_TEXT_SEC); JTextField t = new JTextField(); t.setBackground(new Color(40,40,40)); t.setForeground(Color.WHITE); t.setBorder(BorderFactory.createEmptyBorder(10,10,10,10)); p.add(l, BorderLayout.NORTH); p.add(t, BorderLayout.CENTER); return p; }
    }
    ...
    private void setupTableLogic(JTable table, DefaultTableModel model, Component parent, TableAction action) {
        ...
        table.addMouseListener(new MouseAdapter() { public void mouseExited(MouseEvent e) { table.putClientProperty("hoveredRow", -1); table.repaint(); } public void mouseClicked(MouseEvent e) { int row = table.rowAtPoint(e.getPoint()); int col = table.columnAtPoint(e.getPoint()); if (row != -1) { if (col == model.getColumnCount() - 1) showPopupMenu(e, row, model, parent); else if (action != null) action.onPlay(model.getValueAt(row, 1).toString(), model.getValueAt(row, 2).toString()); } } });
